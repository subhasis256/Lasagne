...
class Conv3DLayer(Layer):
    ...
    incoming : a :class:`Layer` instance or a tuple
        ...
        ``(batch_size, num_input_channels, input_thickness, input_rows, input_columns)``.
    ...
    W : Theano shared variable, numpy array or callable
        An initializer for the weights of the layer. This should initialize the
        layer weights to a 4D array with shape
        ``(num_filters, num_input_channels, filter_thickness, filter_rows, filter_columns)``.
        See :func:`lasagne.utils.create_param` for more information.
    ...
    **kwargs
    ...
    def __init__(self, incoming, num_filters, filter_size, stride=(1, 1, 1),
        ...
        if pad != 'valid':
            raise ValueError, """Only 'valid' pad mode is supported for now"""
...
#            self.pad = as_tuple(pad, 2, int)
    ...
    def get_output_shape_for(self, input_shape):
        ...
        output_thickness = conv_output_length(input_shape[2],
                                              self.filter_size[0],
                                              self.stride[0],
                                              pad[0])
        ...
        return (input_shape[0], self.num_filters, output_thickness, output_rows, output_columns)
...
#                               input_shape[3] + pad[1][0] + pad[1][1])
        ...
        input_dimshuffled = input.dimshuffle(0, 2, 1, 3, 4)

        input_shape_dimshuffled = (input_shape[0], input_shape[2],
                                   input_shape[1], input_shape[3],
                                   input_shape[4])
        filter_shape_dimshuffled = (filter_shape[0], filter_shape[2],
                                    filter_shape[1], filter_shape[3],
                                    filter_shape[4])

        conved = self.convolution(input_dimshuffled, filters,
                                  signals_shape=input_shape_dimshuffled,
                                  filter_shape=filter_shape_dimshuffled,
                                  border_mode='valid')

        if self.b is None:
            activation = conved
        elif self.untie_biases:
            activation = conved + self.b.dimshuffle('x', 1, 0, 2, 3)
        else:
            activation = conved + self.b.dimshuffle('x', 'x', 0, 'x', 'x')
